          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {departments.map((department) => (
                <span
                  key={department.id}
                  className="inline-flex items-center gap-3 rounded-xl border bg-[#eff2fb] px-4 py-2 text-base font-medium text-[#4a5fb0]"
                >
                  {department.name}
                  <form action={archiveDepartmentAction}>
                    <input type="hidden" name="department_id" value={department.id} />
                    <button type="submit" className="text-[#8a97bf] hover:text-destructive" title="Прибрати підрозділ">
                      ×
                    </button>
                  </form>
                </span>
